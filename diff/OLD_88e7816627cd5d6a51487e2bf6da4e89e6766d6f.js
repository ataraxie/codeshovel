...
($ || jQuery).extend(PocketQuery, (function($) {
	...
	function updateUsageListItems(queryName, templateName, converterName, databaseName) {
	...
	function addToUsageListItems(queryName, templateName, converterName, databaseName) {
		var $usageLists = $('.usage-display .expand-content ul');
	...
	function removeFromUsageListItems(queryName) {
		var $itemsToRemove = $('.usage-display .expand-content li[data-queryName=' + queryName + ']');
	...
	function onRemoveItemClick(e) {
		...
		confirm(getText('admin.confirm.remove'), function() {
			...
			pocketPost('pocket-remove.action', params, function(data) {
				...
				if (entityType === 'query') {
					removeFromUsageListItems(entityName);
				}
