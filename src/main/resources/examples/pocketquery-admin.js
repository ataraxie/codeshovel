...
($ || jQuery).extend(PocketQuery, (function($) {
	...
	function onFormSubmit(e) {
		...
		function newEntityIsDuplicate() {
			var isDuplicate = false;
			var entityName = $form.niceEntityName();

			if ($form.attr('data-new-entity') === 'true') {
				$form.closest('.nice-box').find('.nice-list').children().each(function () {
					if ($(this).attr('data-entityname') === entityName) {
						isDuplicate = true;
						return false; // == break;
					}
				});
	...
	function onEditItemClick(e) {
		...
		confirmLostChanges(function() {
			...
			$box.find('form').attr('data-new-entity', 'false');
		});
	}
	...
	function onAddItemClick(e) {
		...
		confirmLostChanges(function() {
			...
			$box.find('form').attr('data-new-entity', 'true');
		});
	}
