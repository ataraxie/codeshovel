...
($ || jQuery).extend(PocketQuery, (function($) {
	...
	var CONFLUENCE_BASE_URL = AJS.Meta.get('base-url');
	var SELECTIVE_EXPORT_ACTION_LINK = '/pocketquery/exportselected.action';
	...
	$.fn.niceClean = function() {
		...
		$form.find('.expand-content').addClass('hidden');
		$form.find('.expand-content li').addClass('hidden');
		$form.find('.export-button').addClass('hidden');
		$form.removeClass('changes');
		$form.niceStatus('');
		$form.niceEnable();

		$('li[data-section='+$form.niceEntityType()+']').removeClass('unsaved-changes');
	};
	...
	function yesNoDialog(header, text, yesFn, noFn) {
		var dialog = new AJS.Dialog({
			width: 400,
			height: 270,
			id: 'nice-yesno-dialog'
		});
		dialog.addHeader(header);
		dialog.addSubmit(getText('yes'), function() {
			dialog.remove();
			yesFn();
		});
		dialog.addSubmit(getText('no'), function() {
			dialog.remove();
			noFn();
		});
		yesFn && dialog.addCancel(getText('cancel'), function() {
			dialog.remove();
		});
	...
	function isEditMode() {
		return !!$getActiveBox().find('.nice-right .nice-form input[name="entityName"]').val();
	}
	...
	function showActiveForm(shallShow) {
		...
		if (shallShow) {
			...
			$activeBox[isEditMode() ? 'removeClass' : 'addClass']('add');
	...
	function onFormSubmit(e) {
		...
		var newEntityIsDuplicate = function() {
			return !isEditMode() && !!PocketQuery.getEntity($form.niceEntityType(), $form.niceEntityName());
		};
		var getDuplicateErrorMessage = function() {
			return getText('admin.error.entity.already.exists', $form.niceEntityType());
		};

		e.preventDefault();
		if (newEntityIsDuplicate()) {
			$form.niceStatus(getDuplicateErrorMessage(), true);
		} else {
			doit();
		}
	}
	...
	function onBeforeUnload() {
		if ($('.nice-form').hasClass('changes')) {
			return getText('admin.confirm.lost.changes');
		}
	}
	...
	function redirectToQueryExport($clickedTarget, includeRelated) {
		var $form = $clickedTarget.closest('form');
		var queryName = $form.niceEntityName();
		var exportActionUrl = 'exportselected.action?type=query&name=' + queryName;
		if (includeRelated) {
			exportActionUrl += '&includeRelated=true';
		}
		$form.niceStatus('');
	...
	function updateExportLink($form, entityType, entityName) {
		var $exportLink = $form.find('.export-button a');
		var target = CONFLUENCE_BASE_URL + SELECTIVE_EXPORT_ACTION_LINK + '?type=' + entityType + '&name=' + entityName;

		$exportLink.attr('href', target);

		$form.find('.export-button').removeClass('hidden');
	}
	...
	function handleAddEditClick($box, entityType, entityName) {
		...
		if (entityName) { // edit
			...
			updateExportLink($form, entityType, entityName);
		} else { // add
	...
	function updateDashboardUi() {
		$('.nice-box[data-section="dashboard"]').html(soyAlias.dashboard({
			...
			userCanAccessDatabases: PocketQuery.getSettings('userCanAccessDatabases')
		}));
	...
	function onQueryExportClick(e) {
		var $target = $(e.target);
		e.preventDefault();
		yesNoDialog(getText('admin.confirm.exportquery.header'), getText('admin.confirm.exportquery'), function() {
			redirectToQueryExport($target, true);
		}, function() {
			redirectToQueryExport($target, false);
		});
	}
	...
	function onFormContentChange() {
		var entityType = $getActiveBox().niceEntityType();
		$('li[data-section='+entityType+']').addClass('unsaved-changes');
	...
	function onExpandTriggerClick(e) {
		e.preventDefault();
		$(e.target).closest('.usage-display').find('.expand-content').slideToggle();
	}
	...
	function bind() {
		$container
			.on('submit', 'form.nice-form', onFormSubmit)
			...
			.on('click', '.expand-trigger', onExpandTriggerClick)
			...
			.on('click', '#pocket-dashboard .clickable-entity-box', onDashboardBoxClick)
			...
			.on('click', '.export-button.export-query a', onQueryExportClick);
